---
title: "12. Marker Gene Method Comparison(Supplemental Figure S39)"
...
DE_comparisons_scatter_plots <- function(tissue, tissue_name, legend_pos=c(0.2, 0.2)) {  
  ...
  umap_grid <- column_label_1 + column_label_2 + column_label_3 + column_label_4 +
    cell_ontology + louvain_default + louvain_callback + louvain_countsplit + 
...
marker_gene_venn_diagram <- function(callback_marker_genes, countsplit_marker_genes, clusterDE_marker_genes) {
  p <- ggVennDiagram(list("recall+ZIP"=callback_marker_genes,
...
# for callback cluster 0 is "skeletal muscle satellite cell" and cluster 1 is "mesenchymal stem cell"
callback_markers_0vs1 <- FindMarkers(limb_muscle, ident.1 = 0, ident.2 = 1, group.by = "callback_idents", min.pct = 0, logfc.threshold = 0)
```
...
```{r scatterplots}
countsplit_vs_callback_markers_plot <- compare_markers_p_vals(countsplit_markers_0vs1,
                                                              callback_markers_0vs1,
...
clusterDE_vs_callback_markers_plot <- compare_markers_q_vals(clusterDE_res$summaryTable, callback_markers_0vs1,
...
ggsave("countsplit_vs_callback_markers_plot.png", countsplit_vs_callback_markers_plot, width = 12, height = 12, units = "in")
ggsave("clusterDE_vs_callback_markers_plot.png", clusterDE_vs_callback_markers_plot, width = 12, height = 12, units = "in")
```

![](countsplit_vs_callback_markers_plot.png){width=100%}
![](clusterDE_vs_callback_markers_plot.png){width=100%}


Plot the Venn diagram
```{r plot_venn}
countsplit_signficant <- countsplit_markers_0vs1[countsplit_markers_0vs1$p_val_adj < 0.05, ]
callback_signficant <- callback_markers_0vs1[callback_markers_0vs1$p_val_adj < 0.05, ]

marker_venn_diagram <- marker_gene_venn_diagram(rownames(callback_signficant), rownames(countsplit_signficant), clusterDE_res$DEgenes)
...
```{r downsampled}
Idents(limb_muscle) <- limb_muscle@meta.data$callback_idents
...
callback_downsampled_indices <- c(cluster_0_downsampled_indices, cluster_1_downsampled_indices)

limb_muscle_downsampled <- limb_muscle[, callback_downsampled_indices]

callback_downsampled_markers <- FindMarkers(limb_muscle_downsampled,ident.1 = 0, ident.2 = 1, min.pct = 0, logfc.threshold = 0)

callback_downsampled_vs_counsplit_markers_plot <- compare_markers_p_vals(countsplit_markers_0vs1,
                                                                         callback_downsampled_markers,
                                                                         xlabel = "countsplit",
...
ggsave("callback_downsampled_vs_counsplit_markers_plot.png", callback_downsampled_vs_counsplit_markers_plot, width = 12, height = 12, units = "in")
```

![](callback_downsampled_vs_counsplit_markers_plot.png){width=100%}
